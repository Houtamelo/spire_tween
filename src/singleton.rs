...
impl TweensController {
	fn tick_process(&mut self, delta_time: f64) {
		let is_tree_paused =
			self.base()
			    .get_tree()
			    .is_none_or(|tree| tree.is_paused());
	...
	pub(crate) fn kill_boundeds(&mut self, bound_node: Gd<Node>) {
		self.tweens
		    .retain(|_, tween| {
			    tween.bound_node()
			         .as_ref()
	...
	pub(crate) fn complete_boundeds(&mut self, bound_node: Gd<Node>) {
		self.tweens
		    .extract_if(|_, tween| {
			    tween.bound_node()
			         .as_ref()
